...
from app.plugins.torrentclassifierenhance.classifierconfig import ClassifierConfig, TorrentFilter, TorrentTarget
...
class TorrentClassifier(_PluginBase):
    ...
    # 插件ID
    plugin_id = "TorrentClassifierEnhance"
    # 插件描述
    plugin_desc = "通过匹配种子属性进行自定义分类。"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/9606/MoviePilot-Plugins/main/icons/TorrentClassifierEnhance.png"
    # 插件版本
    plugin_version = "1.0"
    # 插件作者
    plugin_author = "9606"
    # 作者主页
    author_url = "https://github.com/9606"
    # 加载顺序
    plugin_order = 50
    # 可使用的用户级别
    auth_level = 1
    ...
    def torrent_classifier(self):
        ...
        with lock:
            for service in self.service_infos.values():
                ...
                logger.info(f"开始扫描下载器 {downloader_name} ...")
                ...
                if self._apply_all_rules:
                    self.__apply_rules_to_each_seeds(downloader=downloader_obj)
                else:
    ...
    def __should_classifier(self, config: ClassifierConfig, torrent: Any) -> (bool, str):
        ...
        # 继续检查其他过滤条件
        return self.__check_filters(torrent_filter, torrent_title, torrent_category, torrent_tags, torrent_path)
    ...
    @staticmethod
    def __check_filters(torrent_filter, torrent_title, torrent_category, torrent_tags, torrent_path) -> (bool, str):
        """应用过滤条件检查是否需要整理"""
        ...
        if torrent_filter.torrent_path:
            try:
                if not torrent_path:
    ...
    def __get_torrent_auto_category(self, torrent: Any) -> bool:
        ...
        """
        try:
            return torrent.get("auto_tmm", False)
        except Exception as e:
            print(str(e))
            return False
    ...
    def __get_torrent_path(self, torrent: Any) -> Optional[str]:
        ...
        """
        try:
            return torrent.get("save_path", None)
        except Exception as e:
            print(str(e))
            return None
